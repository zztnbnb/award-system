...
function createContext(options = {}, root = node_process.default.cwd()) {
	...
	root = slash(root);
	const { dts: preferDTS = (0, local_pkg.isPackageExists)("typescript"), dtsMode = "append", dtsPreserveExts = false, dirsScanOptions, dirs, vueDirectives, vueTemplate } = options;
	...
	async function generateDTS(file) {
		...
		let currentContent = await unimport$1.generateTypeDeclarations({ resolvePath: (i) => {
			if (i.from.startsWith(".") || (0, node_path.isAbsolute)(i.from)) {
				const related = slash(dtsPreserveExts ? (0, node_path.relative)(dir, i.from) : (0, node_path.relative)(dir, i.from).replace(/\.ts(x)?$/, ""));
	...
	return {
		...
		vite: {
			...
			async handleHotUpdate({ file }) {
				...
				if (ctx.normalizedDirPaths.some((dirPath) => picomatch.default.isMatch(normalizedFilePath, dirPath.glob))) await ctx.scanDirs();
			},
